#https://github.com/DNXLabs/terraform-aws-log-exporter/tree/master
  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
        ...
        "Resource": "${aws_s3_bucket.log_central_bucket.arn}/*" 
    },
    {
...
resource "aws_lambda_function" "log_exporter" {
  ...
  environment {
    variables = {
      S3_BUCKET = aws_s3_bucket.log_central_bucket.id,
